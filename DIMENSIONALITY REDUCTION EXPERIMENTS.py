...
train = pd.read_csv('./DBS/train.csv')
train.head()
...
cum_var_exp = np.cumsum(var_exp) # Cumulative sum variance
...
fig.layout.xaxis = dict(range=[0, 90], title = 'Feature columns')
fig.layout.yaxis = dict(range=[0, 70], title = 'Explained Variance')
...
# Set a KMeans clustering with 10 components ( 10 chosen sneakily ;) as hopefully we get back our 10 class labels)
